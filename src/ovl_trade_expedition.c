...
void render_trade_market(void) {
    ...
    for (i = 0; i < FOREIGN_TRADE_ENTRY_COUNT; ++i) {
        ...
        print_int_right_aligned_currency(14, PRICE_BOX_Y1 + 1 + i, state.foreign_nations[nation_index].import_prices[i]);
        ...
        print_int_right_aligned_currency(33, PRICE_BOX_Y1 + 1 + i, state.foreign_nations[nation_index].export_prices[i]);
    }
